<nav x-data="{ open: false }" class="bg-green-300 border-b border-green-400">
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            ...
                            <x-dropdown-link :href="route('logout')"
                                    onclick="event.preventDefault();
                                                this.closest('form').submit();" class="text-green-800 hover:bg-green-300">
                                {{ __('Log Out') }}
                            </x-dropdown-link>
                        </form>
                    </x-slot>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')" class="text-green-800 hover:bg-green-200">
                {{ __('Dashboard') }}
        ...
        <div class="pt-4 pb-1 border-t border-green-300">
            ...
            <div class="mt-3 space-y-1">
                <x-responsive-nav-link :href="route('profile.edit')" class="text-green-800 hover:bg-green-200">
                    {{ __('Profile') }}
                </x-responsive-nav-link>
                ...
                <form method="POST" action="{{ route('logout') }}">
                    ...
                    <x-responsive-nav-link :href="route('logout')"
                            onclick="event.preventDefault();
                                        this.closest('form').submit();" class="text-green-800 hover:bg-green-200">
                        {{ __('Log Out') }}
                    </x-responsive-nav-link>
                </form>
            </div>
